    ...
    return (
        <div className="relative" ref={ref}>
            <button onClick={() => setOpen(!open)}
                className="flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 px-2 py-1 rounded-lg transition-colors">
                    ...
                    {!showCustom ? (
                        <>
                            ...
                            {SNOOZE_OPTIONS.map(opt => (
                                <button key={opt.label} onClick={() => handleOption(opt)}
                                    className="w-full text-left px-3 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-indigo-600 transition-colors flex items-center gap-2">
                        ...
                        <div className="p-3 space-y-2">
                            ...
                            <input type="date" value={customDate} onChange={e => setCustomDate(e.target.value)} min={new Date().toISOString().split('T')[0]}
                                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                            <div className="flex gap-2">
                                ...
                                <button onClick={() => customDate && snooze(new Date(customDate))} disabled={!customDate}
                                    className="flex-1 text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white py-1.5 rounded-lg disabled:opacity-50 transition-colors">Set</button>
                            </div>
                        </div>
                    )}
                </div>
